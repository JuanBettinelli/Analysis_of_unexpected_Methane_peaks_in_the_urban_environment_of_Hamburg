...
for (Wind_Provider in 1:4){
  ...
  for(j in seq(0, 27, by = 1)){
    ...
    for (i in seq(0, 350, by = 10)){
      ...
      if (nrow(CH4Filtered2) > 2) {
        r <- cor.test(CH4Filtered2$X.CH4., CH4Filtered2$Water_Level, use="complete.obs")
  ...
  Correlation_Plot <- ggplot()+
    ...
    labs(x = "Wind direction. °", y = "Wind speed, m/s", title = "Pearson's correlation coefficient for Methan and Elbe Waterlevel, depending on Wind Direction and Speed")
  ...
  P_value_Plot <- ggplot()+
    ...
    labs(x = "Wind direction. °", y = "Wind speed, m/s", title = "P-value Test for Methan and Elbe Waterlevel, depending on Wind Direction and Speed")+
