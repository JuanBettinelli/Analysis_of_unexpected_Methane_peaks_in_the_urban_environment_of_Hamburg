...
for (Wind_Provider in 1:4){ 
  ...
  for (k in Water_Names) {
    ...
    for(i in 1:nrow(CH4Data)) {       # for-loop over all rows
      ...
      CH4Data[i , k] <- mean(TotalData[TotalData$UTC >= (CH4Data[i,"UTC"] - 10*60) & TotalData$UTC <= (CH4Data[i,"UTC"] + 10*60), k ], na.rm = TRUE)
    }
    ...
    for(j in seq(0, 27, by = 1)){
      ...
      for (i in seq(0, 350, by = 10)){
        ...
        if (nrow(CH4Filtered2) > 5) {
          r <- cor.test(CH4Filtered2$X.CH4., CH4Filtered2[,k], use="complete.obs")
