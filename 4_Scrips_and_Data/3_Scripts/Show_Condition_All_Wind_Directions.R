...
FinishTime <- as.POSIXct('2021-09-05 22:30:00', 
                         format = "%Y-%m-%d %H:%M:%S", 
                         tz ="utc")
...
for (l in 1:nrow(TotalData)){
  if (TotalData[l,"Water_Level"] < 500 & TotalData[l,"Direction"] > 0 & TotalData[l,"Direction"] < 90 & TotalData[l,"Speed"]>0 & TotalData[l,"Speed"]<6 & TotalData[l,"LIDAR_BLH"]<700){ #   &
  ...
  if (Panel_Plot == FALSE) {
    ...
    for (i in (0:(m-1))){
      ...
      CH4_TimeLine <- ggplot(CH4Data[CH4Data$panel == i, ], aes(x = UTC, y = X.CH4.)) +
        ...
        geom_rect(data = CH4Data[CH4Data$Condition2 == 1 & CH4Data$panel == i, ], aes(xmin = UTC - 60, xmax = UTC + 60, ymin = -Inf, ymax = Inf),
                  fill = "green", alpha = 0.2)+
        geom_rect(data = CH4Data[CH4Data$Condition3 == 1 & CH4Data$panel == i, ], aes(xmin = UTC - 60, xmax = UTC + 60, ymin = -Inf, ymax = Inf),
                  fill = "blue", alpha = 0.2)+
        geom_rect(data = CH4Data[CH4Data$Condition4 == 1 & CH4Data$panel == i, ], aes(xmin = UTC - 60, xmax = UTC + 60, ymin = -Inf, ymax = Inf),
                  fill = "red", alpha = 0.2) #+
      # scale_fill_manual(values = c("yellow", "green","blue", "red"),
      #                   labels = c("0-90°", "90-180°", "180-270°", "270-360°")) +  # Change the legend labels
