...
CH4_Peak_Finder <- function(TotalData = TotalData, Export_CSV = TRUE){
  ...
  for (k in 1:nrow(CH4_Peaks)){
    testDFUp <- filter(TotalData, TotalData$UTC > (CH4_Peaks[k,2]) & TotalData$UTC < (CH4_Peaks[k,2]+6*60*60), .preserve = FALSE)
    testDFUp <- testDFUp[complete.cases(testDFUp[ , "X.CH4.Diff"]),]
    CH4_Up <- as.data.frame(findpeaks(-testDFUp$X.CH4.Diff, npeaks = 1, sortstr=TRUE))
    if (nrow(CH4_Up) == 0){
    ...
    testDFDown <- filter(TotalData, TotalData$UTC > (CH4_Peaks[k,2]-6*60*60) & TotalData$UTC < (CH4_Peaks[k,2]), .preserve = FALSE)
    testDFDown <- testDFDown[complete.cases(testDFDown[ , "X.CH4.Diff"]),]
    CH4_Down <- as.data.frame(findpeaks(-testDFDown$X.CH4.Diff, npeaks = 1, sortstr=TRUE))
    if (nrow(CH4_Down) == 0){
  ...
  if (Export_CSV){
    write.csv(CH4_Peaks, "4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/CH4_Peaks_Large.csv", row.names=TRUE)
    write.csv(colMeans(CH4_Peaks[,c(1,5:29)], na.rm = TRUE), "4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/CH4_PeaksMean_Large.csv", row.names=TRUE)
    write.csv(colMeans(TotalData[,2:27], na.rm = TRUE), "4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/CH4_TotalMean_Large.csv", row.names=TRUE)
  }
  else {
    return(CH4_Peaks)
  }
   ...
   if (Panel_Plot == FALSE) {
     ...
     for (i in (0:(m-1))){
       ...
       ggsave(paste0("4_CH4_Timeline_Wind_Large",i,".png"),
              CH4_TimeLine,
              path = "4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/CH4_Timeline",
              width = 10,
              height = 5)
     }
   }
 ...
 CH4_TimeLine_Diff <- function(TotalData = TotalData, StartTime = StartTime, FinishTime = FinishTime, n = 10, Panel_Plot = FALSE){
   # n<- 10
   # Panel_Plot <- FALSE
   # calling funktions to splite timeline into Panels
   TotalData <- panel_function(TotalData, n)
   ...
   #Select the Data from dataframe with CH4 Concentration
   CH4Data <- TotalData[complete.cases(TotalData[ , "X.CH4."]),c("UTC", "X.CH4.", "X.CH4.Diff", "panel")]
   
   # Find the Methan Peaks
   CH4_Peaks <- CH4_Peak_Finder(TotalData, FALSE)
   CH4_Peaks$panel <- CH4Data[match(CH4_Peaks$UTC, CH4Data$UTC),"panel"]
   CH4_min <- min(CH4Data$X.CH4.Diff, na.rm = TRUE)
   CH4_max <- max(CH4Data$X.CH4.Diff, na.rm = TRUE)
   ...
   if (Panel_Plot == FALSE) {
     ...
     for (i in (0:(m-1))){
       # Create the Timeline plot
       CH4_TimeLine <- ggplot(CH4Data[CH4Data$panel == i, ], aes(x = UTC, y = X.CH4.Diff)) +
         geom_line() +
         labs(x = "Fill Time [UTC]",
              y = expression("CH"[4]*" concentration [ppb]"),
              title = "Methane concentration vs. Time") +
       ...
       ggsave(paste0("4_CH4_Timeline_Wind_Large",i,".png"),
              CH4_TimeLine,
              path = "4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/CH4_Timeline_Diff",
              width = 10,
              height = 5)
     }
   }
   ...
   # Select a plot with seperate panels
     ...
     # Create the Plot
     CH4_TimeLine <- ggplot(CH4Data, aes(x = UTC, y = X.CH4.Diff)) +
       geom_line() +
       labs(x = "Fill Time [UTC]",
            y = expression("CH"[4]*" concentration [ppb]"),
            title = "Methane concentration vs. Time") +
 ...
 WindRose_Plots <- function(TotalData = TotalData){
   ...
   png("4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/WindRosePlots/WindRose_Total_Large.png")
   ...
   png("4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/WindRosePlots/WindRose_Peaks_Large.png")
   ...
   png("4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/WindRosePlots/PollutionRose_Total_Large.png")
   ...
   png("4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/WindRosePlots/PollutionRose_Peaks_Large.png")
   ...
   png("4_Data/OutputData/Plots/23_Diff_Peakfinder_Large/WindRosePlots/Comparison_Total_Vs_Peaks_Large.png")
...
CH4_TimeLine_Diff(TotalData, StartTime, FinishTime, 10, FALSE) #CH4_TimeLine(ImputDataFrame = , StartTime = , FinishTime =, MumberOfPanels = (0=FixedPanelForPaper), TURE = OnePlotMultiplePanels FALSE = MultipePlotsOnePanel)
