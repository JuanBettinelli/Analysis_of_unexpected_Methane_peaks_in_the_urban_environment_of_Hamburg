...
for (Wind_Provider in 1:4){
  ...
  for(i in 1:nrow(CH4Data)) {       # for-loop over all rows
    ...
    CH4Data[i , "pressure_air_site"] <- mean(TotalData[TotalData$UTC >= (CH4Data[i,"UTC"] - 10*60) & TotalData$UTC <= (CH4Data[i,"UTC"] + 10*60), "pressure_air_site" ], na.rm = TRUE)
  }
  ...
  Correlation_Plot <- ggplot()+
    ...
    labs(x = "Wind direction in °", y = "Wind speed in m/s", title = "Pearson's correlation coefficient between methane and air preasure, binned by wind direction and wind speed")
  ...
  P_value_Plot <- ggplot()+
    ...
    labs(x = "Wind direction in °", y = "Wind speed in m/s", title = "5% P-value test for methane and air preasure, binned by wind direction and wind speed")+
    scale_fill_discrete(name = "P-Test Results")
  
  ggsave(paste0("13_CH4_vs_pressure_P_Value_",W_Name, ".png") , P_value_Plot, path = "4_Data/OutputData/Plots/13_Correlation", width = 10, height = 5)
