...
for (Wind_Provider in 1:4){
  ...
  Correlation_Plot <- ggplot()+
    geom_rect(data = Correlation_Data,aes(xmin = Direction_min, ymin = Speed_min, xmax = Direction_max, ymax = Speed_max,
                                          fill = Correlation))+
    labs(x = "Wind direction. °", y = "Wind speed, m/s", title = "Pearson's correlation coefficient for Methan and dew point, depending on Wind Direction and Speed")
  ...
  P_value_Plot <- ggplot()+
    ...
    labs(x = "Wind direction. °", y = "Wind speed, m/s", title = "P-value Test for Methan and dew point, depending on Wind Direction and Speed")+
