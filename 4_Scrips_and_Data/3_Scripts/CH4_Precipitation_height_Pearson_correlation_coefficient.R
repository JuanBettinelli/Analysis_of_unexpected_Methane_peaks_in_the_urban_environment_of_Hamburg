...
for (Wind_Provider in 1:4){
  ...
  for(i in 1:nrow(CH4Data)) {       # for-loop over all rows
    ...
    CH4Data[i , "precipitation_height"] <- mean(TotalData[TotalData$UTC >= (CH4Data[i,"UTC"] - 10*60) & TotalData$UTC <= (CH4Data[i,"UTC"] + 10*60), "precipitation_height" ], na.rm = TRUE)
  }
  ...
  for(j in seq(0, 27, by = 1)){
    ...
    for (i in seq(0, 350, by = 10)){
      ...
      if (nrow(CH4Filtered2) > 5) {
        r <- cor.test(CH4Filtered2$X.CH4., CH4Filtered2$precipitation_height, use="complete.obs")
  ...
  Correlation_Plot <- ggplot()+
    geom_rect(data = Correlation_Data,aes(xmin = Direction_min, ymin = Speed_min, xmax = Direction_max, ymax = Speed_max,
                                          fill = Correlation))+
    labs(x = "Wind direction. °", y = "Wind speed, m/s", title = "Pearson's correlation coefficient for Methan and precipitation height, depending on Wind Direction and Speed")
  
  ggsave(paste0("13_CH4_vs_precipitation_height_Correlation_",W_Name, ".png") , Correlation_Plot, path = "4_Data/OutputData/Plots/13_Correlation", width = 10, height = 5)
  
  P_value_Plot <- ggplot()+
    geom_rect(data = Correlation_Data,aes(xmin = Direction_min, ymin = Speed_min, xmax = Direction_max, ymax = Speed_max,
                                          fill = ifelse(p_value>0.05, 'Fail', 'Pass')))+
    labs(x = "Wind direction. °", y = "Wind speed, m/s", title = "P-value Test for Methan and precipitation height, depending on Wind Direction and Speed")+
    scale_fill_discrete(name = "P-Test Results")
  
  ggsave(paste0("13_CH4_vs_precipitation_height_P_Value_",W_Name, ".png") , P_value_Plot, path = "4_Data/OutputData/Plots/13_Correlation", width = 10, height = 5)
