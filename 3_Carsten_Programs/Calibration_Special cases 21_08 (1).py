...
date_start = '20210821' #format YYYYMMDD as string
date_end = '20210821' #End da
...
cols_GPS_43 = ['time','latitude','longitude','accuracy (m)','altitude (m)','speed (m/s)']
...
for da in date_list:
    ...
    """
    #GPSpath_G23 = "GPS\\Car\\"
    GPSpath_G43 = "GPS\\Car\\"# directory of GPS data of the car
    #G23_dir = "G2301\\" + day + "\\"
    G43_dir = "G4302\\" + day + "\\"
...
#GPS = GPS_.assign(DATE = DATE)
    ...
    GPS_43['time'] = pd.to_datetime(GPS_43['time'], format='%Y-%m-%d %H:%M:%S').dt.time #creating a datetime column 
    GPS_43.rename({'time':'TIME'}, axis=1,inplace=True) # rename the column for consistancy
    #print(GPS['TIME'])
