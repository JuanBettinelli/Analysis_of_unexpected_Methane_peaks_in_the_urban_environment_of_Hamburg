...
date_start = '20210809' #format YYYYMMDD as string
date_end = '20210819' #End da
...
cols_GPS = ['time','latitude','longitude','accuracy (m)','altitude (m)','speed (m/s)']
...
def gps_read(date):
    ...
    for file in files:
        if fnmatch.fnmatch(file, date+'-*.txt'):
            GPS_ = pd.read_csv(GPSpath+file,usecols=cols_GPS)
            break
    return GPS_
...
for da in date_list:
    ...
    """
    GPSpath = "GPS\\Car\\" # directory of GPS data of the car
    G23_dir = "G2301\\" + day + "\\"
    G43_dir = "G4302\\" + day + "\\"
    ...
    """
    GPS = gps_read(date)
    GPS.rename({'accuracy (m)':'accuracy','altitude (m)':'altitude','speed (m/s)':'speed'},axis=1, inplace=True)
...
#GPS = GPS_.assign(DATE = DATE)
    ...
    """
    GPS['time'] = pd.to_datetime(GPS['time'], format='%Y-%m-%d %H:%M:%S').dt.time #creating a datetime column 
    GPS.rename({'time':'TIME'}, axis=1,inplace=True) # rename the column for consistancy
    #print(GPS['TIME'])
    G2301['TIME'] = (pd.to_datetime(G2301['TIME'], format='%H:%M:%S.%f') - timedelta(seconds=delayG23_list[i])).dt.time#creating a datetime column for the merging creating a datetime.time object
    G4302['TIME'] = (pd.to_datetime(G4302['TIME'], format='%H:%M:%S.%f') - timedelta(seconds=delayG43_list[i])).dt.time#creating a datetime column for the merging
    ...
    """
    G23_ = G2301[(G2301['TIME']>= GPS.TIME.iloc[0]) & (G2301['TIME']<= GPS['TIME'].iloc[-1])]
    G43_ = G4302[(G4302['TIME']>= GPS.TIME.iloc[0])&(G4302['TIME']<= GPS['TIME'].iloc[-1])]

    """
    # Merging the Piccaro files with the GPS files
    """
    G23 = merge_interpolate(G23_,GPS,'TIME')
    G43 = merge_interpolate(G43_,GPS,'TIME')
